...
export function getPreset() {
    const presetName = layoutConfig.preset as keyof typeof presets;
    const presetModule = presets[presetName] || Aura;

    return definePreset(presetModule, getPresetExt());
}

/*export function getSemantic() {
    const primarySetting = primaryColors.value.find(
        (color) => color.name === layoutConfig.primary
    );

    return {
        primaryPalette: primarySetting ? primarySetting.palette : {},
...
export function applyTheme(type: 'primary' | 'surface', color: any) {
    if (type === 'primary') {
